...
| 기술                           |    버전    | 선택 및 활용 이유                                                                              |
| :--------------------------- | :------: | :-------------------------------------------------------------------------------------- |
|   React                      | `18.3.1` | LiveKit React v2와 호환되는 안정적인 조합을 유지하고, 프로젝트 중 React 19 마이그레이션보다 실시간 통화·Canvas 기능 안정화에 집중 |
|   TypeScript                 |  `5.6.3` | 그림판 이벤트, 참가자 상태, 실시간 메시지 구조를 타입으로 관리해 복잡한 통화 흐름에서 발생할 수 있는 오류를 줄임                       |
|   Vite                       |  `6.4.3` | 브라우저 중심 React SPA에서 빠른 개발 환경과 ESM 기반 빌드를 구성하기 위해 사용                                     |
|   LiveKit Client             | `2.21.0` | WebRTC 영상통화와 Data Packet 기반 실시간 데이터 통신을 하나의 Room에서 처리                                   |
|   LiveKit Components React   | `2.9.23` | LiveKit의 Room·Participant·Track 상태를 React 생명주기와 연동하기 위해 사용                              |
...
| 핵심 구현           | 적용 내용                                                                            |
| :-------------- | :------------------------------------------------------------------------------- |
|   데이터 전송 분리     | 그림 선처럼 누락되면 결과가 달라지는 데이터는 안정적인 전달을, 커서처럼 최신 위치가 중요한 데이터는 빠른 반응성을 우선하도록 전송 방식을 분리 |
...
| 기술                         |    버전    | 선택 및 활용 이유                                                                          |
| :------------------------- | :------: | :---------------------------------------------------------------------------------- |
|   React                    | `18.3.1` | 영상통화 화면과 필터 상태를 안정적으로 관리하고 기존 LiveKit React 환경과의 호환성을 유지                            |
|   TypeScript               |  `5.6.3` | 얼굴 랜드마크, 필터 상태, 영상별 추론 상태를 타입으로 관리해 런타임 오류 가능성을 줄임                                  |
|   Vite                     |  `6.4.3` | MediaPipe WASM 및 브라우저 기반 추론을 사용하는 React SPA의 개발·빌드 환경으로 활용                          |
|   MediaPipe Tasks Vision   |  `1.0.0` | 브라우저에서 Face Landmarker를 실행하고, JavaScript 패키지와 WASM 런타임 버전을 동일하게 유지해 런타임 불일치 가능성을 줄임 |
...
|   추론 입력 축소       | 화면에 표시되는 원본 영상은 유지하면서 추론용 입력의 긴 변을 최대 `480px`로 축소해 연산량을 감소         |
|   추론 스케줄링        | Local·Remote 영상을 동시에 분석하지 않고 번갈아 추론하도록 처리해 CPU·GPU의 순간적인 연산 부하를 완화 |
